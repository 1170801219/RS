  ...
  def withinCoreSelection(coreID: Int, n1: Int, alpha: Double, delta: Double, param: String, 
    ...
    while(sysContainer.size()>1){
      ...
      for(i<- 0 to numOfComGroups-1){
        ...
        val ppac = new PPAC(alpha/Math.pow(2D,round.toDouble),1.0/3.0*Math.pow(0.75,round.toDouble)*delta,param,cov,n1,inGroupContainer);
        ppac.runSystem();
        
        acc_sim += ppac.getTotalSample();
        acc_sim_t += ppac.getSimTime();
        acc_com_t += ppac.getCompareTime();
        
        val tempans = new SOAnswer();
        tempans.setSeedString(ppac.getBestAnswer().getSeedString());
        
        surviveContainer.put(ppac.getBestID(),tempans);
        inGroupContainer.clear();
      ...
      surIDs.clear();
      surviveContainer.clear();
      remain = remain - 1.0/3.0*Math.pow(0.75,round.toDouble)*delta;
      round += 1;
    }
    ...
    var rinottSampleSize = Math.ceil(sysContainer.get(surIDs.get(0)).getFnVar()*Math.pow(rinottH/remain,2)).toInt-n1;
    
    if (rinottSampleSize<0){
      rinottSampleSize = 0;
    }
